...
const createUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { error } = userValidation_1.userSignup.validate(userData);
    if (error) {
...
const loginUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { error } = userValidation_1.userLogin.validate(userData);
    if (error) {
        return res
            .status(400)
    ...
    try {
        ...
        if (user) {
            ...
            if (!isValid) {
                ...
                const jwtSecret = process.env.JWT_SECRET || "secret";
                const tokenExpire = process.env.TOKEN_EXPIRES || "2h";
                const token = jsonwebtoken_1.default.sign({ userId: user._id, userName: user.userName, email: user.email }, jwtSecret, {
                    expiresIn: tokenExpire,
                });
                res.status(200).header("Authorization", `Bearer ${token}`).send({
                    data: token,
